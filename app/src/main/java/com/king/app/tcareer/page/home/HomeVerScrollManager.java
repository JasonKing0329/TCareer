...
public class HomeVerScrollManager implements NestedScrollView.OnScrollChangeListener {
    ...
    private int[] getColor(int position) {
        ...
        if (court.equals(AppConstants.RECORD_MATCH_COURTS[1])) {
            color = colorClay;
        } else if (court.equals(AppConstants.RECORD_MATCH_COURTS[2])) {
            color = colorGrass;
        } else if (court.equals(AppConstants.RECORD_MATCH_COURTS[3])) {
            color = colorInhard;
        } else {
            color = colorHard;
        }
